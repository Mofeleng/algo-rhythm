...
import { ClockIcon, DownloadIcon, GlobeIcon, LoaderIcon, MoreHorizontal, MusicIcon, PencilIcon, PlayIcon, PlusIcon, RefreshCcwIcon, SearchIcon, TrashIcon, XCircle } from "lucide-react";
...
export function ManageSongsView({ songs }: { songs: Song[] }) {
    ...
    const { mutate: handleGeneratePlayUrl } = useGeneratePlayUrl();
    ...
    return (
        <div className="px-4">
            {selectedId && (
                <>
                    <DeleteSongAlert open={isDeleteOpen} setOpen={setIsDeleteOpen} songId={selectedId} />
                    <RenameSongModal open={isRenameOpen} setOpen={setIsRenameOpen} songId={selectedId} prevName={prevName ?? ""} />
                </>
            )}
            {user && (
                <NewSongModal open={isCreateOpen} setOpen={setIsCreateOpen} userId={user.id} />
            )}
                    ...
                    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button
                                ...
                                className="cursor-pointer flex-1 sm:flex-none"
                            >
                                <RefreshCcwIcon className={cn("mr-2 h-4 w-4", isRefreshing && "animate-spin")} />
                                Refresh
                            </Button>
                        </div>
                    </div>
                        ...
                        {filteredSongs.length === 0 && (
                            <div className="mt-3 w-full text-center">
                                <p className="text-muted-foreground text-sm">
                                    No songs yet. Your song generation history will show here.
                                </p>
                            </div>
                        )}
                                    ...
                                    return (
                                        <div key={song.id} className="flex cursor-not-allowed items-center gap-4 rounded-lg p-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-muted-foreground text-xs">Please try creating the song again</p>
                                            </div>
                                        </div>
                                    );
                                    ...
                                    return (
                                        <div key={song.id} className="flex cursor-not-allowed items-center gap-4 rounded-lg p-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-muted-foreground text-xs">You have reached your song generation limit</p>
                                            </div>
                                        </div>
                                    );
                                ...
                                default:
                                    return (
                                        <div
                                            ...
                                            className="hover:bg-muted/50 flex cursor-pointer items-center p-3 gap-3 rounded-lg"
                                        >
                                            {/* Thumbnail */}
                                            <div className="group relative h-12 w-12 shrink-0 overflow-hidden rounded-md">
                                                {song.thumbnailUrl ? (
                                                    <img src={song.thumbnailUrl} alt="Song thumbnail" className="h-full w-full object-cover" />
                                                ) : (
                                            ...
                                            {/* Title + prompt — takes all remaining space, truncates */}
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    <h3 className="truncate text-sm font-medium">{song.title}</h3>
                                                    {song.instrumental && (
                                                        <Badge variant="outline" className="shrink-0">Instrumental</Badge>
                                                    )}
                                                </div>
                                                <p className="text-muted-foreground truncate text-xs mt-0.5">{song.prompt}</p>
                                            </div>

                                            {/* Actions — shrink-0 so they never squeeze the title */}
                                            <div
                                                className="flex items-center gap-1 shrink-0"
                                                onClick={(e) => e.stopPropagation()}
                                            >
                                                {/* Publish: icon-only on mobile, labelled on sm+ */}
                                                <Button
                                                    onClick={() => handleSetPublishSong(song.id)}
                                                    variant="outline"
                                                    size="sm"
                                                    className={cn(
                                                        "cursor-pointer hidden xs:flex sm:flex",
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-44">
                                                        ...
                                                        <DropdownMenuItem
                                                            onClick={() => {
                                                                ...
                                                                handleGeneratePlayUrl(song.id, {
                                                                    onSuccess: (data) => window.open(data.songUrl, "_blank"),
                                                                });
                                                            }}
                                                        >
                                                        ...
                                                        <DropdownMenuItem onClick={() => handleToggleRename(song.id, song.title)}>
                                                            <PencilIcon className="mr-2 h-4 w-4" />
                                                            Rename
                                                        </DropdownMenuItem>

                                                        <DropdownMenuItem onClick={() => handleToggleDelete(song.id)}>
